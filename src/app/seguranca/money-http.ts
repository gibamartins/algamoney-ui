...
// Classe que vai representar uma exceção, quando o refresh-token expirar.
// Isto se faz necessário, por que o TypeScript não reconhece mais o instanceof
// da classe AuthHttpError, que estende de Error, entre outras.
export class NotAuthenticatedError {}

/*
* Classe que tem a funcionalidade de sobrescrever todos os métodos da classe AuthHttp,
* para verificar a validade do token e, caso inválido, chama o método para renovação.
*/
@Injectable()
export class MoneyHttp extends AuthHttp {
  ...
  private fazerRequisicao(fn: Function): Observable<Response> {
    if (this.auth.isAccessTokenInvalido()) {
      ...
      const chamadaNovoAccessToken = this.auth.obterNovoAccessTokenComRefreshToken()
        .then(() => {
          if (this.auth.isAccessTokenInvalido()) {
